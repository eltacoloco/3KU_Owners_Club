...
static void lcd_prepare_menu()
{
	...
	MENU_ITEM(gcode, "3KU Level", PSTR("G88"));
    END_MENU();
}
...
static void lcd_move_z()
{
    if (encoderPosition != 0)
    {
        ...
        encoderPosition = 0;
        #ifdef DELTA
		if (max_software_endstops && current_position[Z_AXIS] > nozzel_hight)
           current_position[Z_AXIS] = nozzel_hight;
        calculate_delta(current_position);
...
//3ku
static void lcd_loadMaterial(){
	
	
}
//3ku
static void lcd_unloadMaterial(){
...
static void lcd_move_e()
{
    if (encoderPosition != 0)
    {
        ...
        #ifdef DELTA
        calculate_delta(current_position);
        plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS], manual_feedrate[E_AXIS]/30, active_extruder);
        #else
        plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], manual_feedrate[E_AXIS]/30, active_extruder);
        #endif
        lcdDrawUpdate = 1;
    }
    if (lcdDrawUpdate)
...
static void lcd_control_motion_menu()
{
    START_MENU();
    MENU_ITEM(back, MSG_CONTROL, lcd_control_menu);
	MENU_ITEM_EDIT(float32, "Level Thick ", &level_thickness, 0.1, 10);
