...
float nozzel_hight=MANUAL_Z_HOME_POS;
float level_thickness=LEVEL_Z_OFFSET;
#endif
...
void setup()
{
   //laser init by 3ku--changkai
  #if defined(LASERPIN_5V) && LASERPIN_5V > -1
    SET_OUTPUT(LASERPIN_5V);
    WRITE(LASERPIN_5V,LOW);
  #endif
  setup_killpin();
  setup_powerhold();
...
static void axis_is_at_home(int axis) {
  ...
  if(axis==Z_AXIS){
        current_position[axis] = nozzel_hight+ add_homeing[axis]-level_thickness;//by Kevin Chang;
        min_pos[axis] =          base_min_pos(axis) + add_homeing[axis];
       max_pos[axis]= nozzel_hight + add_homeing[axis]-level_thickness;//by Kevin Chang
   }else{
...
void process_commands()
{
  ...
  if(code_seen('G'))
  {
    ...
    case 28: //G28 Home all Axis one at a time
      ...
      endstops_hit_on_purpose();
      break;
    case 88: //G88 3ku changkai auto_level
    {
      SERIAL_ECHOLNPGM("Begin 3KU leveling !");
      SERIAL_ECHOLNPGM("Clean level adjusting !");
      endstop_adj[0]=0;
      endstop_adj[1]=0;
      endstop_adj[2]=0;
       saved_feedrate = feedrate;
      ...
      destination[Z_AXIS] = 3 * Z_MAX_LENGTH;
      feedrate = 1.732 * homing_feedrate[X_AXIS];
      plan_buffer_line(destination[X_AXIS], destination[Y_AXIS], destination[Z_AXIS], destination[E_AXIS], feedrate/120, active_extruder);
      st_synchronize();
      endstops_hit_on_purpose();
      ...
      calculate_delta(current_position);
      plan_set_position(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS]);
//      destination[Z_AXIS]=Z_MAX_LENGTH-120;
//      calculate_delta(destination);
//      plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS],current_position[E_AXIS],1.732 * homing_feedrate[X_AXIS]/120,active_extruder);
//      st_synchronize();
//      endstops_hit_on_purpose();
      long start_steps = st_get_position(Z_AXIS);
      feedrate = homing_feedrate[Z_AXIS];
      ...
      endstops_hit_on_purpose();
      long stop_steps = st_get_position(Z_AXIS);
      float mm = float(start_steps - stop_steps) / axis_steps_per_unit[Z_AXIS];
     // plan_buffer_line(0.0,0.0,0.0,0.0,1.732 * homing_feedrate[X_AXIS]/200,active_extruder);
      SERIAL_ECHOPAIR("Current height is",mm);
      SERIAL_ECHOLN("");     
       //
         // Move all carriages up together until the first endstop is hit.
      current_position[X_AXIS] = 0;
    ...
    // destination[i] = (float)code_value() + (axis_relative_modes[i] || relative_mode)*current_position[i];
      ...
      //check X axis
      destination[X_AXIS]=-86.6+ (axis_relative_modes[X_AXIS] || relative_mode)*current_position[X_AXIS];
      destination[Y_AXIS]=-50+ (axis_relative_modes[Y_AXIS] || relative_mode)*current_position[Y_AXIS];
      feedrate= homing_feedrate[X_AXIS];
      prepare_move();
      st_synchronize();
      endstops_hit_on_purpose();
      calculate_delta(destination);
      plan_set_position(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS]);
       long x_start_steps = st_get_position(Z_AXIS);
      feedrate = homing_feedrate[Z_AXIS];
      destination[Z_AXIS] = -10;
      prepare_move_raw();
      st_synchronize();
      endstops_hit_on_purpose();
      long x_stop_steps = st_get_position(Z_AXIS);
      float x_mm = (float(x_start_steps - x_stop_steps) / axis_steps_per_unit[Z_AXIS])-15;
      SERIAL_ECHOPAIR("x distance is :",x_mm);
       SERIAL_ECHOLN("");
      current_position[X_AXIS]= destination[X_AXIS];
      current_position[Y_AXIS]= destination[Y_AXIS];
      current_position[Z_AXIS]=0;
      calculate_delta(current_position);
      plan_set_position(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS]);
      destination[Z_AXIS]=x_mm+15;
      prepare_move_raw();
      st_synchronize();
      //check Y axis
       destination[X_AXIS]=88+ (axis_relative_modes[X_AXIS] || relative_mode)*current_position[X_AXIS];
      destination[Y_AXIS]=-49+ (axis_relative_modes[Y_AXIS] || relative_mode)*current_position[Y_AXIS];
       feedrate = homing_feedrate[Z_AXIS];
      prepare_move();
      st_synchronize();
      endstops_hit_on_purpose();
      calculate_delta(destination);
      plan_set_position(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS]);
      long y_start_steps = st_get_position(Z_AXIS);
      feedrate = homing_feedrate[Z_AXIS];
      destination[Z_AXIS] = -10;
      prepare_move_raw();
      st_synchronize();
      endstops_hit_on_purpose();
      long y_stop_steps = st_get_position(Z_AXIS);
      float y_mm = (float(y_start_steps - y_stop_steps) / axis_steps_per_unit[Z_AXIS])-15;
      SERIAL_ECHOPAIR("y distance is :",y_mm);
       SERIAL_ECHOLN("");
      current_position[X_AXIS]= destination[X_AXIS];
      current_position[Y_AXIS]= destination[Y_AXIS];
      current_position[Z_AXIS]=0;
      calculate_delta(current_position);
      plan_set_position(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS]);
      destination[Z_AXIS]=y_mm+15;
      prepare_move_raw();
      st_synchronize();
      //check Z axis
       destination[X_AXIS]=-4+ (axis_relative_modes[X_AXIS] || relative_mode)*current_position[X_AXIS];
      destination[Y_AXIS]=97+ (axis_relative_modes[Y_AXIS] || relative_mode)*current_position[Y_AXIS];
       feedrate = homing_feedrate[Z_AXIS];
      prepare_move();
      st_synchronize();
      endstops_hit_on_purpose();
      calculate_delta(destination);
      plan_set_position(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS]);
      long z_start_steps = st_get_position(Z_AXIS);
      feedrate = homing_feedrate[Z_AXIS];
      destination[Z_AXIS] = -10;
      prepare_move_raw();
      st_synchronize();
      endstops_hit_on_purpose();
      long z_stop_steps = st_get_position(Z_AXIS);
      float z_mm = (float(z_start_steps - z_stop_steps) / axis_steps_per_unit[Z_AXIS])-15;  
      SERIAL_ECHOPAIR("z distance is :",z_mm);
      ...
      {
        adjust=x_mm;
        if(adjust>y_mm) adjust=y_mm;
        if(adjust>z_mm) adjust =z_mm;
      }
      
      endstop_adj[0]=adjust-x_mm;
      endstop_adj[1]=adjust-y_mm;
      endstop_adj[2]=adjust-z_mm;
      nozzel_hight=mm+adjust;
      max_pos[Z_AXIS]=nozzel_hight-level_thickness;

      Config_StoreSettings();

      SERIAL_ECHOLNPGM("End 3KU leveling !");
       SERIAL_ECHOLN("");
       LCD_MESSAGEPGM("3KU Level finished !");
      break; 
    }
#ifdef ENABLE_AUTO_BED_LEVELING
  ...
  else if(code_seen('M'))
  {
    ...
    break;
#endif   
    case 3: //M3 Open laser by changkai
         #if defined(LASERPIN_5V) && LASERPIN_5V > -1
             WRITE(LASERPIN_5V,HIGH);
         #endif
    break; 
    case 5://M5 close laser by changkai --3KU
         #if defined(LASERPIN_5V) && LASERPIN_5V > -1
             WRITE(LASERPIN_5V,LOW);
         #endif
    break; 
...
void ck_mc_arc(float *position, float *target, float *offset, uint8_t axis_0, uint8_t axis_1, 
...
{      
  ...
  */
  ...
  for (i = 1; i<segments; i++) { // Increment (segments-1)
    ...
    // start calculte by changkai
    float difference[NUM_AXIS];
    for (int8_t i=0; i < NUM_AXIS; i++) {
      difference[i] = arc_target[i] - current_position[i];
    }
    float cartesian_mm = sqrt(sq(difference[X_AXIS]) +
                              sq(difference[Y_AXIS]) +
                              sq(difference[Z_AXIS]));
    ...
    for (int s = 1; s <= steps; s++) {
      float fraction = float(s) / float(steps);
      for(int8_t i=0; i < NUM_AXIS; i++) {
        arc_target[i] = current_position[i] + difference[i] * fraction;
      }
      ...
      #endif
      plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS],
                       arc_target[E_AXIS], feedrate*feedmultiply/60/100.0,
                       active_extruder);
                
    }  
      for(int8_t i=0; i < NUM_AXIS; i++) {
        current_position[i] = arc_target[i];
      }
  ...
  // Ensure last segment arrives at target location.
   ...
   // start calculte by changkai
    ...
    for (int s = 1; s <= steps; s++) {
      ...
      #endif
      plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS],
                       target[E_AXIS], feedrate*feedmultiply/60/100.0,
                       active_extruder);
...
void prepare_arc_move(char isclockwise) {
  ...
  // Trace the arc
  ck_mc_arc(current_position, destination, offset, X_AXIS, Y_AXIS, Z_AXIS, feedrate*feedmultiply/60/100.0, r, isclockwise, active_extruder);
