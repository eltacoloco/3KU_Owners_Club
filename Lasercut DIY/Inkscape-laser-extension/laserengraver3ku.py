#!/usr/bin/env python 
"""
Modified by 3ku ---Changkai -kaichang365@hotmail.com
...
'header': """
;Created by 3KU Tech.
;author: changkai(Kevin)
;QQ 277499536 
;www.3k-3dprinter.com
;3kaiu.taobao.com
;mail kaichang365@hotmail
;skype: qingniaozou
G28 X0
G90
G21
G0 Z80 F3000
""",
'footer': """G0 X0.000 Y0.000 F1000.00
M05
G28
;3KU
"""
...
def biarc(sp1, sp2, z1, z2, depth=0):
    ...
    if options.disableArc  or (d > 1 and depth<options.biarc_max_split_depth )    : return biarc_split(sp1, sp2, z1, z2, depth)
    else:
...
class Laserengraver3ku(inkex.Effect):
    ...
    def __init__(self):
        ...
        self.OptionParser.add_option("",   "--x-offset",        action="store", type="float",         dest="x_offset", default="0",                    help="X Offset")
        self.OptionParser.add_option("",   "--y-offset",        action="store", type="float",         dest="y_offset", default="0",                    help="Y Offset")
        self.OptionParser.add_option("",   "--z-offset",        action="store", type="float",         dest="z_offset", default="0",                    help="Z Offset Object height")
        ...
        self.OptionParser.add_option("",   "--biarc-max-split-depth",        action="store", type="int",         dest="biarc_max_split_depth", default="1",            help="Defines maximum depth of splitting while approximating using biarcs.")  
        self.OptionParser.add_option("",   "--disable-Arc", action="store", type="inkbool",    dest="disableArc", default=False,help="IF disable the arc function G2 G3.")   		
...
#                    print_((-f(w[k][i-1]),-f(w[k][i]), [i1[5] for i1 in l1]) )
    ...
    def check_dir(self):
        ...
        if (os.path.isdir(self.options.directory)):
            ...
            #self.header += "G92 X"+str(self.options.x_offset*-1)+" "
            self.header += "Y"+str(self.options.y_offset*-1)+" "
            self.header += "Z"+str(self.options.z_offset*-1)+"\n"
        ...
        if self.options.active_tab not in ['"Laser"', '"orientation"']:
            ...
            if self.options.active_tab == '"Laser"':
                ...
                self.tools = {
                    ...
                    "xoffset": self.options.x_offset,
                    "yoffset": self.options.y_offset,
                    "zoffset": self.options.z_offset,
                    "in trajectotry": "",
                    "out trajectotry": "",
                    "gcode before path": "\nM400\nM03",
